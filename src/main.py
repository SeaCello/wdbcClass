...
with open(f'files/results.txt', 'w') as f:
    for name, model, param_grid, scale_data in modelos:
        ...
        plt.grid()
        plt.plot(train_sizes, train_scores_mean, 'o-', color="#c5002a", label="Training score")
        plt.plot(train_sizes, test_scores_mean, 'o-', color="#1f77b4", label="Cross-validation score")
        plt.legend(loc="best")
